...
public class JakartaInProxy<T> implements CriteriaBuilder.In<T> {
    private final javax.persistence.criteria.CriteriaBuilder.In<T> actual;

    public JakartaInProxy( javax.persistence.criteria.CriteriaBuilder.In<T> actual )
    {
        this.actual = actual;
    }

    @Override public Expression<T> getExpression()
    {
        return new JakartaExpressionProxy<>( actual.getExpression() );
    }

    @Override public CriteriaBuilder.In<T> value( T value )
    {
        return new JakartaInProxy<>( actual.value( value ) );
    }

    @Override public CriteriaBuilder.In<T> value( Expression<? extends T> value )
    {
        return new JakartaInProxy<>( actual.value( new JavaXExpressionProxy<>( value ) ) );
    }
