...
public class JavaXJoinProxy<X, T> implements Join<X, T> {
    private final jakarta.persistence.criteria.Join<X, T> actual;

    public JavaXJoinProxy( jakarta.persistence.criteria.Join<X, T> actual )
    {
        this.actual = actual;
    }

    @Override public Join<X, T> on( Expression<Boolean> expression )
    {
        return null;
    }

    @Override public Join<X, T> on( Predicate... predicates )
    {
        return null;
    }
