...
public class WebEndpointConfig implements Jackson2ObjectMapperBuilderCustomizer {
    ...
    public <T extends Identifiable<I>,I> SpringMVCEntityURLBuilderImpl<T,I> mvcEntityHATOASURLBuilder( SpringMVCEndPointRegistry mvcEndPointRegistry) {
        return new SpringMVCEntityURLBuilderImpl<>(linkBuilderFactory(), mvcEndPointRegistry);
    }

    @Bean
    public <T extends Identifiable<I>,I> SpringMVCEntityURLBuilderImpl<T,I> mvcEntityURLBuilder(SpringMVCEndPointRegistry mvcEndPointRegistry) {
        return new SpringMVCEntityURLBuilderImpl<>(linkBuilderFactory(), mvcEndPointRegistry);
    }

    @Bean
    public <T extends Identifiable<I>,I> ResponseEntityBuilderFactory<T, ResponseEntity<T>> mvcResponseEntityBuilderHATOASFactory(SpringMVCEntityURLBuilderImpl<T, I> mvcEntityHATOASURLBuilder) {
        return new SpringMVCResponseEntityBuilderFactory<>(mvcEntityHATOASURLBuilder);
    }

    @Bean
    public <T extends Identifiable<I>,I> ResponseEntityBuilderFactory<T,ResponseEntity<T>> mvcResponseEntityBuilderFactory(SpringMVCEntityURLBuilderImpl<T,I> mvcEntityURLBuilder) {
